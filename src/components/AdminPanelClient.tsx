    ...
    <div className="min-h-screen bg-primary-dark text-white p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-6">
          ...
          {/* Tab Navigation */}
          <div className="flex bg-primary-darker p-1 rounded-full border border-white/10">
            <button
              onClick={() => setActiveTab('products')}
              className={`px-6 py-2 rounded-full text-sm font-bold transition ${activeTab === 'products' ? 'bg-accent-gold text-primary-dark' : 'text-gray-400 hover:text-white'}`}
            >
              Products
            </button>
            <button
              onClick={() => setActiveTab('orders')}
              className={`px-6 py-2 rounded-full text-sm font-bold transition ${activeTab === 'orders' ? 'bg-accent-gold text-primary-dark' : 'text-gray-400 hover:text-white'}`}
            >
              Orders
            </button>
          </div>
        ...
        {activeTab === 'products' ? (
          ...
          <div className="space-y-6">
            {loading ? <p className="text-center text-gray-500">Loading orders...</p> : orders.length === 0 ? (
              <p className="text-center text-gray-500 py-12">No orders found.</p>
            ) : (
              orders.map(order => (
                <div key={order.id} className={`${CARD_CLASS} p-6 border-l-4 ${order.status === 'new' ? 'border-l-accent-gold' : 'border-l-green-500'}`}>
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        <h3 className="text-xl font-bold text-white">Order #{order.orderId}</h3>
                        {order.status === 'new' && (
                          <span className="px-2 py-0.5 rounded text-xs font-bold bg-accent-gold text-primary-dark uppercase animate-pulse">New</span>
                        )}
                        {order.status === 'confirmed' && (
                          <span className="px-2 py-0.5 rounded text-xs font-bold bg-green-500/20 text-green-400 uppercase">Confirmed</span>
                        )}
                      </div>
                      <p className="text-sm text-gray-400">{new Date(order.createdAt).toLocaleString()}</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <p className="text-xl font-bold text-accent-gold">${order.totalPrice.toFixed(2)}</p>
                      <button
                        onClick={() => setExpandedOrder(expandedOrder === order.id ? null : order.id)}
                        className="p-2 hover:bg-white/5 rounded-lg text-gray-400 transition"
                      >
                        <svg className={`w-6 h-6 transform transition ${expandedOrder === order.id ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <div className="space-y-3">
                            {order.cartItems.map((item, idx) => (
                              <div key={idx} className="flex items-center gap-3 bg-black/20 p-2 rounded-lg">
                                ...
                                <p className="text-sm font-bold text-accent-gold">${(item.price * item.quantity).toFixed(2)}</p>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {/* {order.status === 'new' && (
                        <div className="mt-8 flex justify-end">
                          <button
                            ...
                            className="px-6 py-2 bg-green-600 text-white font-bold rounded-lg hover:bg-green-500 transition shadow-lg shadow-green-900/20"
                          >
                            Confirm Order
                          </button>
                        </div>
